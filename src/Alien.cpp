...
void Alien::Update(float dt){
	...
	/*int spd = 256;
    associated.angleDeg =  arc1 * 180 / PI - 90;
	arc1 -= 2 * PI * 15 * dt/750;
	if (arc1 > 2 * PI){
        arc1 -= 2 * PI;
    }
	...
	if (state == RESTING) {
		...
		if (restTimer.Get() > 5) {
			if (PenguinBody::player != nullptr) {
				...
				speed = (destination - associated.box.GetVecCentralizado()).Normalizado();
				speed.x*= 0.5;
	...
	if (state == MOVING) {
		...
		if ((associated.box.GetVecCentralizado() - destination).Magnitude() < dt) {
			destination = PenguinBody::player->associated.box.GetVecCentralizado();
			Minion* selected = nullptr;
			float min = FLT_MAX;
